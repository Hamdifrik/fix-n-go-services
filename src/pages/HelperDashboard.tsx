...
const HelperDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (userStr) {
      try {
        ...
        // Vérifier que c'est bien un helper
        if (userData.role !== 'helper') {
          navigate('/dashboard'); // Rediriger vers le dashboard client
          return;
        }
        setUser(userData);
      } catch (error) {
  ...
  const sidebarItems = [
    ...
    { id: 'available', icon: Search, label: 'Missions disponibles', badge: availableMissions.length, path: null },
    { id: 'my-missions', icon: FileText, label: 'Mes missions', badge: myMissions.length, path: null },
    { id: 'messages', icon: MessageSquare, label: 'Messages', badge: 2, path: null },
    { id: 'wallet', icon: Wallet, label: 'Portefeuille', path: '/helper/profile' },
