...
const NewHelperDashboard = () => {
  ...
  const myServices = servicesResponse?.data || [];
  ...
  const statsData = [
    { label: 'Services actifs', value: stats?.activeServices?.toString() || myServices.length.toString(), icon: Package, color: 'text-primary' },
    { label: 'Revenus du mois', value: `${stats?.totalEarnings || 0}€`, icon: Euro, color: 'text-secondary' },
    { label: 'Réservations', value: stats?.totalBookings?.toString() || '0', icon: CalendarIcon, color: 'text-warning' },
    { label: 'Terminées', value: stats?.completedBookings?.toString() || '0', icon: CheckCircle2, color: 'text-info' },
  ];
